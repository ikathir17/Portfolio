...
import { X, Home, User, FolderOpen, Settings, Mail, Sparkles } from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
    <header
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled && !isNavigating
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 md:px-8 flex justify-between items-center py-4 sm:py-6">
        ...
        <nav className="hidden md:flex space-x-6 lg:space-x-8">
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={item.action}
              className={`text-sm uppercase tracking-wider transition-all duration-300 relative group ${activeSection === item.id
                ? 'text-blue-400'
                : 'text-white hover:text-white/90'
                }`}
            >
              <span>{item.label}</span>
              <span className={`absolute -bottom-1 left-0 h-0.5 bg-blue-400 transition-all duration-300 ${activeSection === item.id ? 'w-full' : 'w-0 group-hover:w-full'
                }`}></span>
            </button>
          ))}
        </nav>
        ...
        >
          <div className="relative w-6 h-6">
            <span className={`absolute top-0 left-0 w-6 h-0.5 bg-white transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-2.5' : ''
              }`}></span>
            <span className={`absolute top-2.5 left-0 w-6 h-0.5 bg-white transition-all duration-300 ${isOpen ? 'opacity-0' : ''
              }`}></span>
            <span className={`absolute top-5 left-0 w-6 h-0.5 bg-white transition-all duration-300 ${isOpen ? '-rotate-45 -translate-y-2.5' : ''
              }`}></span>
          </div>
      ...
      <div className={`md:hidden fixed inset-0 z-40 transition-all duration-500 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
        ...
        {/* Menu Content */}
        <div className={`absolute top-0 right-0 w-80 h-full bg-black/95 backdrop-blur-xl border-l border-white/20 transform transition-transform duration-500 shadow-2xl ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}>
          {/* Header */}
            ...
            <div className="space-y-2">
              {menuItems.map((item, index) => (
                <button
                  key={item.id}
                  onClick={item.action}
                  className={`w-full group flex items-center gap-4 p-4 text-left transition-all duration-300 rounded-xl border border-transparent ${activeSection === item.id
                    ? 'text-blue-400 bg-blue-500/10 border-blue-500/20'
                    : 'text-white hover:text-white/90 hover:bg-white/5 hover:border-white/10'
                    }`}
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center transition-colors duration-300 ${activeSection === item.id
                    ? 'bg-blue-500/20'
                  ...
                  <div className={`ml-auto transition-opacity duration-300 ${activeSection === item.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                    }`}>
